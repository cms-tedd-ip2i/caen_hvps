...
class CAEN_Controller:
    ...
    def init(self):
        ...
        print("DEBUG_INIT:::::",self.username, self.password, self.ip_address,self.ip_address.decode("utf-8"),self.device_name, self.system_type, self.link_type)
        ...
        print("communicating with HVPS, check SLOT # and Channel #, IP : %s, ERROR code : %s" % (self.ip_address, hex(return_code)))
    ...
    def deinit(self):
        ...
        try:
            ...
            print("Something didn't go well with de-init'ing, I wish I could tell you more but it's probably not the end of the world.")
        return return_code
    ...
    def get_channel_paramters(self, slot, channel):
        ...
        c_channel_params_num = (c_int * 1)()  # single value int array, it's just what it needs, don't think about it too hard...
        ...
        num_parameters_for_channel = c_channel_params_num[0]  # This is an array and even using only one channel here have to treat it as such
    ...
    def get_all_info_for_channels(self, slot, channels):
        ...
        for my_channel in channels:  # loop over all the channels
            full_channel_parameters_list = []
            channel_info_dict = {}
            channel_name = self. get_channel_names(slot, [my_channel])  # Might as well grab the channel name, wonder what it'll be...

            full_channel_parameters_list = self.get_channel_paramters(slot, my_channel)  # get list of all available parameters
            ...
            for my_parameter in full_channel_parameters_list:
                c_channels_list = (c_ushort * 1)(*[my_channel])  # multi-dimentional c array again
    ...
    def set_channel_parameter(self, slot, channel, parameter, new_value):
        ...
        c_channels_list = (c_ushort * 1)(*[channel])  # Array of 1 as we do this one at a time in this code
    ...
    def set_channel_name(self, slot, channel, channel_name):
        ...
        c_channels_list = (c_ushort * 1)(*[channel])  # again array of 1 as we do these 1 at a time
